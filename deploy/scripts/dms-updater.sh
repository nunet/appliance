...
build_url() {
  local channel="$1"
  echo "https://d.nunet.io/nunet-dms-${ARCH}-${channel}.deb"
}
...
echo "Checking for new DMS version for $ARCH (env=$ENVIRONMENT, channel=$PREFERRED_CHANNEL)..."
...
for channel in "${attempt_channels[@]}"; do
  ...
  if DEBIAN_FRONTEND=noninteractive apt-get --reinstall install -f -y \
